...
std::vector<AnimationPoint> makeRunnerPath(int fromBase, int toBase, double secondsPerBase) {
    ...
    for (int base = startBase; base <= finalBase; ++base) {
        points.push_back(runnerBasePoint(base, secondsPerBase * (base - startBase)));
    }
    return points;
}
...
std::vector<RunnerAnimation> buildRunnerAnimationsFromBases(
    ...
    for (const auto& candidate : candidates) {
        ...
        const double secondsPerBase = result.type == AtBatResultType::Walk ? 0.55 : 0.45;
    ...
    for (const auto& runnerOut : result.runnerOuts) {
        ...
        const double secondsPerBase = 0.45;
    ...
    const std::vector<BaseRunningEvent>& events)
{
    ...
    for (const auto& ev : events) {
        ...
        const double spb = 0.45; // seconds per base
...
Team::Team(std::string teamName, std::vector<Player> lineup,
    ...
    if (lineup_.empty()) {
        throw std::invalid_argument("Team lineup cannot be empty.");
    }
}
...
const Player& Team::starter() const { return starter_; }
...
void GameEngine::tryPitcherChange() {
    ...
    // 接戦 8回+ → Setup
    if (inning >= 8 && isClose && tryBestOfRole(PitcherRole::Setup))       return;
    // 接戦 6-7回 → 最良 MiddleRelief
    if (inning >= 6 && isClose && tryBestOfRole(PitcherRole::MiddleRelief)) return;
    // 通常 8回+ → Setup
...
bool GameEngine::simulateNextPitch() {
    ...
    // バント意図: 状況に応じて設定 (毎球評価)
    {
        ...
        if (buntSituation) {
            ...
            // Lower in order (7-9 = indices 6-8) → more likely to bunt
            const bool weakHitter = battingOrderPos >= 6;
            buntProb = weakHitter ? 0.12 : 0.05;
        }
...
PlayResult GameEngine::buildPlayResult(const AtBatState& atBat) {
    ...
    const PlayResolution resolution = playResolutionEngine_.resolve(
        result.battedBall, state_, random_, pitchingDefenseAlignment(), ballpark_);
...
PlayResult GameEngine::simulateAtBat(const Player& batter, const Player& /*pitcher*/) {
    ...
    const Player formed    = formedBatter(batter, effective);
    const AtBatResult ar = atBatEngine_.simulatePlateAppearance(formed, effective, random_);
        ...
        case AtBatResultType::FlyOut: {
            ...
            // 犠牲フライ: 3塁走者あり・0 or 1 アウト・外野フライ (200ft+)
            {
                ...
                if (sacFlyEligible) {
                    ...
                    result.throwDecision = {4, false, {4}}; // always throw home on sac fly attempt
...
void GameEngine::advanceRunnersForHit(PlayResult& result, int bases) {
    ...
    // Track whether the fielder threw home (contested play at the plate)
    bool threwHome = false;
    ...
    for (int i = 2; i >= 0; --i) {
        ...
        if (bases == 1) {
            if (i == 1) {
                // 2塁走者: 単打でホームへ突っ込むか → 走力と肩の勝負
                const int spd = getRunnerSpeed(runner);
                const double armPen  = (fielderArm - 128.0) * 0.007;
                const double spdBon  = (spd - 55) * 0.006;
                const double pScore    = std::clamp(0.60 + spdBon - armPen, 0.35, 0.84);
                const double pThrowOut = std::clamp(0.010 + armPen * 1.05 - spdBon * 0.35, 0.00, 0.13);
                const double u = random_.real(0.0, 1.0);
                threwHome = true; // fielder always throws home when 2B runner challenges
                if (u < pScore) {
                    destination = 3;
                    battingBoxScore().extraBasesTaken++;   // XBT
                } else if (u < pScore + pThrowOut) {
                    thrownOut = true;
                } else {
                    destination = 2;  // 3塁止まり
                }
            } else if (i == 0) {
                // 1塁走者: 単打で3塁まで行くか (刺殺なし)
                const int spd = getRunnerSpeed(runner);
                const double spdBon = (spd - 55) * 0.007;
                if (random_.chance(std::clamp(0.31 + spdBon, 0.10, 0.62))) {
                    destination = 2;
                    battingBoxScore().extraBasesTaken++;   // XBT
                }
            }
            ...
            if (i == 0) {
                // 1塁走者: 二塁打でホームへ突っ込むか → 走力と肩の勝負
                const int spd = getRunnerSpeed(runner);
                const double armPen  = (fielderArm - 128.0) * 0.007;
                const double spdBon  = (spd - 55) * 0.006;
                const double pScore    = std::clamp(0.52 + spdBon - armPen, 0.30, 0.78);
                const double pThrowOut = std::clamp(0.015 + armPen * 1.05 - spdBon * 0.35, 0.00, 0.15);
                const double u = random_.real(0.0, 1.0);
                threwHome = true; // fielder throws home when 1B runner challenges on double
                if (u < pScore) {
                    destination = 3;
                    battingBoxScore().extraBasesTaken++;   // XBT
                } else if (u < pScore + pThrowOut) {
    ...
    // Set throw decision based on whether a home-plate challenge occurred
    if (threwHome) {
        result.throwDecision = {4, false, {4}};
    } else {
        ...
        result.throwDecision = {targetBase, false, {targetBase}};
    }
}
...
void GameEngine::advanceRunnersForError(PlayResult& result) {
    for (int i = 2; i >= 0; --i) {
        ...
        if (destination >= 3) {
            ...
            state_.bases[static_cast<std::size_t>(destination)] = runner;
            result.events.push_back(runner + " advances to " + std::to_string(destination + 1) + (destination == 1 ? "nd." : "rd."));
        }
    }
...
void GameEngine::checkPickoff() {
    ...
    for (int baseIdx = 0; baseIdx <= 1; ++baseIdx) {
        ...
        // 試み確率: 1Bは 2%、2Bは 0.5%
        const double attemptProb = (baseIdx == 0) ? 0.020 : 0.005;
        ...
        const double outProb    = std::clamp(0.35 - spdFactor + armFactor, 0.15, 0.60);
...
void GameEngine::checkStolenBase() {
    ...
    // 1塁のみ走者 → 2塁盗塁試み
    if (state_.bases[0].has_value() && !state_.bases[1].has_value()) {
        const std::string runner = *state_.bases[0];
        int runnerSpd = 55;
        for (const auto& p : battingTeam().lineup()) {
            if (p.name == runner) { runnerSpd = p.speed; break; }
        }
        ...
        const double attemptProb = std::clamp(0.035 + spdFactor, 0.005, 0.12);

        if (random_.chance(attemptProb)) {
            const double successProb = std::clamp(0.72 + spdFactor - armFactor, 0.45, 0.92);
            const bool   success     = random_.chance(successProb);
            const int    afterPitch  = static_cast<int>(currentAtBat_.pitchLogs.size());

            if (success) {
                state_.bases[0].reset();
                state_.bases[1] = runner;
                logs_.push_back(GameLog{state_.inning, state_.isTop,
                    "SB: " + runner + " steals 2B."});
                currentAtBat_.baseRunningEvents.push_back({
                    BaseRunningEventType::StolenBase, runner, 1, 2, false, afterPitch});
                ...
                state_.outs += 1;
                logs_.push_back(GameLog{state_.inning, state_.isTop,
                    "CS: " + runner + " caught stealing 2B."});
                currentAtBat_.baseRunningEvents.push_back({
                    BaseRunningEventType::CaughtStealing, runner, 1, 2, false, afterPitch});
                for (auto& p : battingPlayerStats()) {
...
void GameEngine::checkWildPitchPassedBall() {
    ...
    const double wpProb = std::clamp(0.020 + (vel - 85.0) * 0.0003, 0.010, 0.040);
    const double pbProb = 0.008;
...
void GameEngine::printFinalScore(std::ostream& out) const {
    ...
    auto printPlayerStats = [&out](const std::vector<PlayerBoxScore>& players,
        ...
        for (const auto& p : players) {
            ...
            const double obp = pa > 0 ? static_cast<double>(p.hits + p.walks) / pa : 0.0;
